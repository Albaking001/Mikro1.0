...
def build_bins(points_xy: list[tuple[float, float]], bin_size: float):
...
def count_in_radius(bins, x: float, y: float, radius: float, bin_size: float) -> int:
    r2 = radius * radius
    ix = int(math.floor(x / bin_size))
    iy = int(math.floor(y / bin_size))
    c = 0
...
def nearest_distance(bins, x: float, y: float, bin_size: float, max_rings: int = 8) -> float | None:
    best = None
...
def compute_heatmap_payload(db: Session, city_name: str, step_m: int, radius_m: int) -> dict:
    ...
    if nx <= 0 or ny <= 0:
        raise ValueError("Computed grid size invalid. Check city bounds or step_m.")
...
def write_fixed_heatmap_file(payload: dict, filename: str = "planning_heatmap.json") -> Path:
