...
def main():
    ...
    try:
        ...
        if nx <= 0 or ny <= 0:
            raise SystemExit("Computed grid size invalid. Check city bounds or step_m.")
        ...
        print(f"[precompute] Fetch OSM (bbox padded) for {city_name} ...")
        ...
        print(f"[precompute] wrote {out_path} ({total} points)")
